...
__all__ = ['vgg_spool', 'get_vgg_spool']

up_kwargs = {'mode': 'bilinear', 'align_corners': True}

class vgg_spool(nn.Module):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, base_size=520, crop_size=480, mean=[.485, .456, .406],
        ...
        super(vgg_spool, self).__init__()
        self.mean = mean
        self.std = std
        self.base_size = base_size
        self.crop_size = crop_size
        self._up_kwargs = up_kwargs
        self.base = vgg_spool_base(norm_layer)
        self.head = vgg_spoolHead(512, nclass, norm_layer, up_kwargs=self._up_kwargs)
...
class vgg_spool_base(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d):
        super(vgg_spool_base, self).__init__()
        self.layer1 = vgg_spool_layer(3,64,1,1,norm_layer)
        self.layer2 = vgg_spool_layer(64,64,1,1,norm_layer)
        self.layer2_spool = vgg_spool_layer3(64,64,1,1,256,256,norm_layer)
        self.pool = nn.MaxPool2d(2)
        
        self.layer3 = vgg_spool_layer(64,128,1,1,norm_layer)
        self.layer4 = vgg_spool_layer(128,128,1,1,norm_layer)
        self.layer4_spool = vgg_spool_layer3(128,128,1,1,128,128,norm_layer)

        self.layer5 = vgg_spool_layer(128,256,1,1,norm_layer)
        self.layer6 = vgg_spool_layer(256,256,1,1,norm_layer)
        self.layer7 = vgg_spool_layer(256,256,1,1,norm_layer)
        self.layer7_spool = vgg_spool_layer3(256,256,1,1,64,64,norm_layer)
        
        self.layer8 = vgg_spool_layer(256,512,1,1,norm_layer)
        self.layer9 = vgg_spool_layer(512,512,1,1,norm_layer)
        self.layer10 = vgg_spool_layer(512,512,1,1,norm_layer)
        self.layer10_spool = vgg_spool_layer3(512,512,1,1,32,32,norm_layer)
        
        self.layer11 = vgg_spool_layer(512,512,1,1,256,256,norm_layer)
        self.layer12 = vgg_spool_layer(512,512,1,1,256,256,norm_layer)
        self.layer13 = vgg_spool_layer(512,512,1,1,256,256,norm_layer)
...
class vgg_spool_layer2(nn.Module):
    def __init__(self, in_planes, out_planes, dilation=1, tl_size=1, norm_layer=nn.BatchNorm2d):
        super(vgg_spool_layer2, self).__init__()
        self.tl_size = tl_size
        self.inplanes = in_planes
        self.outplanes = out_planes
...
def get_vgg_spool(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = vgg_spool(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
