...
class localSR(nn.Module):
    def __init__(self, in_channels, out_channels, up_kwargs):
        ...
        self._up_kwargs = up_kwargs
        self.refine = nn.Sequential(nn.Conv2d(in_channels+out_channels, inter_channels, 3, padding=1, dilation=1, bias=True),
                                   nn.ReLU(),
...
class Boundary(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        ...
        self.connect = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 1, padding=0, dilation=1, bias=False),
                                     norm_layer(inter_channels),
                                   nn.ReLU())
        self.project = nn.Sequential(nn.Conv2d(out_channels, inter_channels, 1, padding=0, dilation=1, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU())
