...
class dfpn87_gsfHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
        ...
        self.project4 = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 3, padding=1, dilation=1, stride=2, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU(),
                                   )
        self.project5 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, padding=1, dilation=1, stride=2, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU(),
                                   )
    ...
    def forward(self, c1,c2,c3,c4):
        _,_, h,w = c2.size()
        out5 = self.project4(c4)
        out6 = self.project5(out5)
        out4 = self.conv5(c4)
        ...
        p3 = F.interpolate(out3, (h,w), **self._up_kwargs)
        p4 = F.interpolate(out4, (h,w), **self._up_kwargs)
        p5 = F.interpolate(out5, (h,w), **self._up_kwargs)
        p6 = F.interpolate(out6, (h,w), **self._up_kwargs)
        p7 = F.interpolate(gp, (h,w), **self._up_kwargs)
        out = self.project(torch.cat([out2,p3,p4,p5,p6,p7], dim=1))
