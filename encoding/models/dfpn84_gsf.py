...
class dfpn84_gsfHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
        ...
        self.decoder = nn.Sequential(nn.Conv2d(304, 256, 3, padding=1, dilation=1, bias=False),
                                   norm_layer(256),
                                   nn.ReLU(),
    ...
    def forward(self, c1,c2,c3,c4):
        ...
        out = self.gff(out)
        _,_,hl,wl = c1.size()
        out2 = self.decoder(torch.cat([F.interpolate(self.project2(out), (hl,wl), **self._up_kwargs), self.skip(c1)], dim=1))
        #
        out = torch.cat([out2, gp.expand_as(out)], dim=1)
