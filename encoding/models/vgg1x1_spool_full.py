...
__all__ = ['vgg1x1_spool_full', 'get_vgg1x1_spool_full']

up_kwargs = {'mode': 'bilinear', 'align_corners': True}

class vgg1x1_spool_full(nn.Module):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, base_size=520, crop_size=480, mean=[.485, .456, .406],
        ...
        super(vgg1x1_spool_full, self).__init__()
        self.mean = mean
        self.std = std
        self.base_size = base_size
        self.crop_size = crop_size
        self._up_kwargs = up_kwargs
        self.base = vgg1x1_spool_full_base(norm_layer)
        self.head = vgg1x1_spool_fullHead(512, nclass, norm_layer, up_kwargs=self._up_kwargs)
...
class vgg1x1_spool_fullHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs=None):
        super(vgg1x1_spool_fullHead, self).__init__()
        self._up_kwargs = up_kwargs
...
class vgg1x1_spool_full_base(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d):
        super(vgg1x1_spool_full_base, self).__init__()
        self.layer1 = vgg1x1_spool_full_layer(3,64,1,1,norm_layer)
        ...
        self.layer13 = vgg1x1_spool_full_layer3(512,512,1,1,256,256,norm_layer)
...
class vgg1x1_spool_full_layer2(nn.Module):
    def __init__(self, in_planes, out_planes, dilation=1, tl_size=1, norm_layer=nn.BatchNorm2d):
        super(vgg1x1_spool_full_layer2, self).__init__()
        self.tl_size = tl_size
        self.inplanes = in_planes
        self.outplanes = out_planes
...
def get_vgg1x1_spool_full(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = vgg1x1_spool_full(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
